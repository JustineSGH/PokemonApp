  ...
  ngOnInit(): void {
    ...
    if(pokemonId){
      this.pokemonService.getPokemonByIdFromGraphQL(pokemonId).subscribe(
        pokemon => {
          const pokemonReceived = pokemon.data.pokemonById
          if(pokemonReceived) {
            this.pokemon = {
              cp: pokemonReceived.cp,
              hp: pokemonReceived.hp,
              created: pokemonReceived.created,
              idPokemon: pokemonReceived.idPokemon,
              name: pokemonReceived.name,
              picture: pokemonReceived.picture,
              types: pokemonReceived.types
            }
          }
